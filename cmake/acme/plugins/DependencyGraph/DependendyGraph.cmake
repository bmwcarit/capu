...
PRINT_DETAILS(STATUS "...requires external tool Dot")
		...
		IF(NOT "${DOXYGEN_DOT_EXECUTABLE}" STREQUAL "DOXYGEN_DOT_EXECUTABLE-NOTFOUND")
			IF(EXISTS ${DOXYGEN_DOT_EXECUTABLE})
				...
				IF (NOT "${dot_run_ret}" STREQUAL "0") 
					MESSAGE(STATUS "ERROR: PNG GENERATION FAILED with code ${dot_run_ret}")
					MESSAGE(STATUS "${dot_run_out}")
				ELSE()
					MESSAGE(STATUS "PNG generation successfull.")
				ENDIF()	
				...
				IF (NOT "${dot_run_ret}" STREQUAL "0") 
					MESSAGE(STATUS "ERROR: MAP GENERATION FAILED with code ${dot_run_ret}")
					MESSAGE(STATUS "${dot_run_out}")
				ELSE()
					MESSAGE(STATUS "MAP generation successfull.")
				ENDIF()	
				
			ELSE()
				MESSAGE(---------------------------------------------------------------------------)
				MESSAGE("")
				MESSAGE(STATUS "WARNING: The dot executable ${DOXYGEN_DOT_EXECUTABLE} doesn't exist.")
			ENDIF()
		ENDIF()	
	ENDIF()
...
FUNCTION(INTERNAL_DOT_ADD_CMAKE_PROJECT global_list)
	FOREACH(entry ${global_list})
		...
		#IF(EXISTS ${PROJECT_SOURCE_DIR}/deliverable/${entry}/doc/html/_full_01dependency_01graph.html)
			FILE(APPEND ${CMAKE_DOC_OUTPUT_DIRECTORY}/dependency_graph.dot \"${entry}\"\ [shape=diamond,\ style=filled,\ fillcolor=grey,\ URL=\"${url}\"]\; \n)
		#ELSE()
		#	FILE(APPEND ${CMAKE_DOC_OUTPUT_DIRECTORY}/dependency_graph.dot ${entry}\ [shape=diamond,\ style=filled,\ fillcolor=grey]\; \n)
		#ENDIF()
	ENDFOREACH()
