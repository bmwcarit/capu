...
// default for little endian
    ...
    {
        EXPECT_EQ(HostOrderValueLong, capu::Byteorder::NtoHl(NetworkOrderValueLong));
    }

    TEST(Byteorder,CanConvert16BitNetworkToHost)
    {
        EXPECT_EQ(HostOrderValueShort, capu::Byteorder::NtoHs(NetworkOrderValueShort));
    }

    TEST(Byteorder,CanConvert32BitHostToNetwork)
    {
        EXPECT_EQ(NetworkOrderValueLong, capu::Byteorder::HtoNl(HostOrderValueLong));
    }

    TEST(Byteorder,CanConvert16BitHostToNetwork)
    {
        EXPECT_EQ(NetworkOrderValueShort, capu::Byteorder::HtoNs(HostOrderValueShort));
    }
}
