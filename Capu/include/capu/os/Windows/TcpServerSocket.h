...
#ifndef CAPU_WINDOWS_TCPSERVERSOCKET_H
#define CAPU_WINDOWS_TCPSERVERSOCKET_H
...
namespace capu
{
    namespace os
    {
        ...
        TcpServerSocket::accept(uint32_t timeoutMillis)
        {
            ...
            capu::TcpSocket* clientSocket = new capu::TcpSocket(socket);
            return clientSocket;
        }
        ...
        TcpServerSocket::bind(uint16_t port, const char* addr)
        {
            ...
            {
                int32_t optVal = 1;
                if (setsockopt(mTcpServerSocket, SOL_SOCKET, SO_REUSEADDR, (char*)&optVal, sizeof(optVal)) != 0)
                {
                    return CAPU_SOCKET_ESOCKET;
                }
            }
            ...
            if (0 == mPort)
            {
                ...
                Memory::Set((char*) &socketAddr, 0x00, sizeof(socketAddr));
                result = ::getsockname(mTcpServerSocket, (sockaddr*) &socketAddr, &size);
                if (result == SOCKET_ERROR)
                {
                    return CAPU_SOCKET_EBIND;
                }
