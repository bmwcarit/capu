...
#ifndef CAPU_BYTEORDER_H
#define CAPU_BYTEORDER_H

#include "capu/os/PlatformInclude.h"
#include CAPU_PLATFORM_INCLUDE(Byteorder)

namespace capu
{

    class Byteorder : private os::arch::Byteorder
    {
    public:
        /**
    ...
    {
        return os::arch::Byteorder::NtoHl(netlong);
    }

    inline uint16_t Byteorder::NtoHs(uint16_t netshort)
    {
        return os::arch::Byteorder::NtoHs(netshort);
    }

    inline uint32_t Byteorder::HtoNl(uint32_t hostlong)
    {
        return os::arch::Byteorder::HtoNl(hostlong);
    }

    inline uint16_t Byteorder::HtoNs(uint16_t hostshort)
    {
        return os::arch::Byteorder::HtoNs(hostshort);
    }

}
#endif // CAPU_BYTEORDER_H
